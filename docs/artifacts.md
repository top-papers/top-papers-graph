...
- в каждом шаге: **sources[]** (можно несколько: text/image/table) + интерпретация + логика вывода
- опционально: **edges** между шагами (ориентированный граф), чтобы затем строить граф знаний

...
3. `notebooks/top_papers_graph_task3_hf_benchmark_colab.ipynb` — канонический Colab uploader для benchmark. Он использует схему: скачать CSV из Google Sheet, выбрать последние отправки по `Timestamp`, скачать файлы из URL-колонок, собрать dataset folder, загрузить folder в Hugging Face Hub.

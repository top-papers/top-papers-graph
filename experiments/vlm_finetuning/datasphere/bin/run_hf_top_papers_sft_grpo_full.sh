...
if [ "${ENABLE_DPO_FIRST_PIPELINE:-1}" = "1" ] && [[ "${OUT_PREFIX:-}" != *smoke* ]]; then
  ...
  export DPO_PRECOMPUTE_REF_LOG_PROBS="${DPO_PRECOMPUTE_REF_LOG_PROBS:-0}"
  export DPO_PRECOMPUTE_REF_BATCH_SIZE="${DPO_PRECOMPUTE_REF_BATCH_SIZE:-2}"
  export DPO_TRAIN_MAX_IMAGES_PER_EXAMPLE="${DPO_TRAIN_MAX_IMAGES_PER_EXAMPLE:-1}"
  export DPO_MAX_PIXELS="${DPO_MAX_PIXELS:-501760}"
  export DPO_MAX_TEXT_CHARS="${DPO_MAX_TEXT_CHARS:-12000}"
  export DPO_TORCH_EMPTY_CACHE_STEPS="${DPO_TORCH_EMPTY_CACHE_STEPS:-5}"

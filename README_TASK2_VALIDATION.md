...
Notebook bootstrap теперь умеет автоматически распаковывать локальный ZIP-архив репозитория (`top-papers-graph*.zip`) рядом с ноутбуком или в `/content`, поэтому ручная распаковка перед запуском больше не обязательна.
